...
export const CheckoutProvider = ({ children }: CheckoutProviderProps) => {
  ...
  const onPaymentCompleted = () => {
    complete(undefined, {
      onSuccess: ({ data }) => {
        resetCart()
        var comment = cart?.context?.comment ?? ''
        var q = window.btoa(JSON.stringify(comment));
        push({ pathname: `/order/confirmed/${data.id}`, query: { q } })
      },
    })
  }
  ...
  return {
    shipping_address: {
      ...
      postal_code:
        ...
        "",
      metadata: { country: String(cart?.shipping_address?.metadata?.country || "") || "" },
      phone:
    ...
    billing_address: {
      ...
      postal_code:
        ...
        "",
      metadata: { country: String(cart?.billing_address?.metadata?.country || "") || "" },
      phone:
